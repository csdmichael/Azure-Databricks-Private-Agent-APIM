...
function Assert-ExistingConnector {
    ...
    if ([string]$oauthSettings.identityProvider -ne 'aad' -or [string]$oauthSettings.clientId -ne [string]$IdentityConnector.clientId -or
        ...
        (Get-CustomParameterValue -Settings $oauthSettings -Name 'EnableOnbehalfOfLogin') -ne 'False') {
        throw "Existing connector '$($Definition.ConnectorName)' OAuth configuration has drifted. Delete and recreate it explicitly."
    }
    $swagger = $Connector.properties.openApiDefinition
    $expectedOperations = @($Definition.Swagger.paths.PSObject.Properties.Name | Sort-Object)
    $actualOperations = @($swagger.paths.PSObject.Properties.Name | Sort-Object)
    if ([string]$swagger.swagger -ne '2.0' -or [string]$swagger.host -ne $gatewayUri.Host -or [string]$swagger.basePath -ne $Definition.BasePath -or
    ...
    foreach ($pathProperty in $Definition.Swagger.paths.PSObject.Properties) {
        $actualPathProperty = $swagger.paths.PSObject.Properties[$pathProperty.Name]
...
try {
    ...
    foreach ($definition in $definitions) {
        ...
        if ($existing.Count -eq 1) {
            $connector = Invoke-RestMethod -Uri "$adminUri/$($existing[0].name)?api-version=2016-11-01" -Headers $powerHeaders
